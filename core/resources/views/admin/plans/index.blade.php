    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-right">
                <a class="btn btn-success round-btn" href="{{ route('admin.plans.create') }}"><i class="fa fa-plus"
                    ...
                    <tbody>
                        @foreach ($datas as $data)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $data->name }}</td>
                                <td>{{ $data->earning_rate }}</td>
                                <td>{{ $data->price }} {{ $settings->cur_sym }}</td>
                                <td>
                                    <a class="btn"
                                        href="{{ $data->status ? route('admin.plans.unapprove', $data->id) : route('admin.plans.approve', $data->id) }}">
                                        <span
                                            class="label label-{{ $data->status ? 'success' : 'inverse' }}">{{ $data->status ? 'Active' : 'Inactive' }}</span>
                                    </a>
                                </td>
                                <td class="btn-sm">
                                    <a class="btn btn-primary" href="{{ route('admin.plans.edit', $data->id) }}"><i
                                            class="fa fa-edit" aria-hidden="true"></i></a>
                                    <button type="button" class="btn btn-danger delBtn" data-toggle="modal"
                                        data-target="#delModal" data-delTitle="{{ $data->name }}"
                                        data-delAction="{{ route('admin.plans.destroy', $data->id) }}">
